...
@Component
public class DirectSignalingHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        if (targetSession != null && targetSession.isOpen()) {
            ...
            if ("CALL".equals(payload.getType()) || "OFFER".equals(payload.getType())) {
                PendingNotification notif = new PendingNotification();
                notif.setReceiverEmail(target);
                notif.setCreatedAt(Instant.now());
                
                String notifPayload = String.format("{\"type\":\"MISSED_CALL\", \"caller\":\"%s\", \"callType\":\"DIRECT\"}", payload.getSenderEmail());
